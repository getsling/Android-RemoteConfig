...
public class JSONMethods {
	...
	public static String readJSONFeedString(String urlString, ArrayList<Pair<String, String>> headers) {
		...
		setHttpTimeoutParams(httpClient);
		try {
			...
			if(statusCode==200) {
				...
				stringResponse = stringFromStream(entity.getContent());
			}
		} catch (Exception e) {
			e.printStackTrace();
		} 
	...
	public static JSONObject readJSONFeed(String urlString, ArrayList<Pair<String, String>> headers) {
		...
		try {
			...
			if(headers != null) {
				for(int i = 0; i<headers.size() ;i++) {
					httpRequest.setHeader(headers.get(i).first, headers.get(i).second);
				}
			}
			httpRequest.setURI(URI.create(urlString));
			HttpResponse response = httpClient.execute(httpRequest);
			...
			if(statusCode==200) {
				HttpEntity entity = response.getEntity();
				jsonResponse = new JSONObject(stringFromStream(entity.getContent()));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} 
